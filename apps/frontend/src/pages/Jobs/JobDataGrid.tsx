...
import { getGradientFromId, messagesFromExtension, messagesToExtension } from 'misc-glob';
...
type ExtractedDataType = ExtractTypeIfPropertyExists<GetJobQuery['user'], 'data'>
...
export const JobDataGrid = () => {
  ...
  const [suitabilityScoreRange, setSuitabilityScoreRange] = useState<[number, number]>([100, 0]);

  useEffect(() => {
    if (data?.user?.__typename !== 'QueryUserSuccess') return;
    const range = data.user.data.job.map((x) => x.suitabilityScore)
      .reduce((acc, curr) => {
        if (!curr) return acc;
    ...
    console.log(`range: ${range}, ${JSON.stringify(data.user.data.job, null, 2)}`);
  ...
  const [currentlySelected, setCurrentlySelected] = useState<Job | undefined>(undefined);
  ...
  useEffect(() => {
    ...
    if (selectionModel.length === 1) {
      ...
      const jobDesc = data.user.data.job.find((x) => x.id === currentlySelectedID);
      if (!jobDesc) return;
  ...
  useEffect(() => {
    if (data?.user?.__typename !== 'QueryUserSuccess') return;
    const selected = data?.user?.data.job.find((x) => x.id === currentlySelected?.id);
    if (selected) {
  ...
  const columns: GridColDef[] = [
    ...
    },
    {
      ...
      renderCell: (params) => (
        <Link to={`/home/experiences/job/${params.row.id}`}>
          <div
    ...
    },
    {
      ...
      // ),
      valueGetter: (params) => {
        if (params.row.suitabilityScore === null || params.row.suitabilityScore === 0) return 0;
        const scaledValue = ((params.row.suitabilityScore - suitabilityScoreRange[0])
          / (suitabilityScoreRange[1] - suitabilityScoreRange[0])) * 9 + 1;
        return parseFloat(scaledValue.toFixed(1));
      },

      renderCell: (params) => (
        <div>
          {params.value !== 0 && `${params.value}/10`}
        </div>
      ),
    },
    {
    ...
    },
    {
      ...
      renderCell: (params) => (
        <Link to={`/home/experiences/job/${params.row.id}`}>
          <div
            className="btn btn-primary h-10 w-10 m-auto p-1"
          >
            Go
        ...
        data.user.data.job.length === 0 && (
          <div className="card bg-info mt-4">
            <div className="card-body">
              ...
              <p className="card-text">You can manually add them, or get the browser extension to quickly add them!</p>
            </div>
          </div>
        )
      }
