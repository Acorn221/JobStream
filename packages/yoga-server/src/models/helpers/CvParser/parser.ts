...
import { matchCriteria, matchesCriteria } from './misc';
...
export const parseParagraph = (paragraph: docsV1.Schema$Paragraph) => {
  ...
  const children = paragraph.elements.reduce((acc, curr) => {
  ...
  if (children.length === 0 || typeof paragraph?.paragraphStyle?.namedStyleType !== 'string') return null;

  // check if the paragraph style is valid as ParagraphStyle
  if (!Object.keys(paragraphStyleMap).includes(paragraph.paragraphStyle.namedStyleType)) {
    return null;
  }

  const tag = paragraphStyleMap[paragraph.paragraphStyle.namedStyleType as ParagraphStyle];

  if (tag) {
    return `<${tag}>${children}</${tag}>`;
  }
  throw new Error(`Unknown paragraph style: ${paragraph.paragraphStyle?.namedStyleType}`);
};
